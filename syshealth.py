...
logging.basicConfig(level=logging.WARNING, format=get_log_format())
...
def check_dependencies() -> None:
    ...
    recommended = config.get(
        "dependencies.recommended", ["lshw", "df", "ps", "cat", "free"]
    )
    ...
    if missing_recommended:
        logger.warning(
            f"Missing recommended dependencies: {', '.join(missing_recommended)}"
        )
        logger.info(
...
def collect_system_info(host: str) -> dict[str, str]:
    ...
    Returns:
        dict[str, str]: A dictionary containing all collected system information keyed by
