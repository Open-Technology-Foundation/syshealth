    ...
    def collect(self) -> dict[str, str]:
        ...
        return {
            ...
            "recent_errors": self._safe_execute(
                self.config.recent_errors_command, "Error logs not available"
            ),
            "auth_failures": self._safe_execute(
                self.config.auth_failures_command, "Authentication logs not available"
            ),
            ...
            "rootkit_check": self._safe_execute(
                self.config.rootkit_check_command, "Rootkit check logs not available"
            ),
